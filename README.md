# 🌴 Sababa Tours — Premium Travel Agency

**Sababa Tours** is a modern luxury travel platform designed to help travelers discover the breathtaking beauty of Bangladesh through curated journeys and immersive experiences. From the misty hills of Bandarban and Sajek Valley to the crystal beaches of Cox’s Bazar and Saint Martin Island, the website showcases premium travel destinations with elegant visuals, interactive booking experiences, and detailed travel information.

Built with React, Tailwind CSS, and Firebase Authentication, Sababa Tours delivers a fast, responsive, and user-friendly experience across all devices. The platform features dynamic tour packages, real-time search and filtering, secure authentication, interactive travel planning, and beautifully crafted UI animations that create a premium digital travel experience.

Whether travelers are looking for adventurous mountain escapes, peaceful coastal retreats, or cultural explorations, Sababa Tours offers handcrafted travel experiences tailored for every kind of explorer.

---

## 🌐 Live Demo

A modern, elegant, and luxury travel website for **Sababa Tours** — a premium travel agency specializing in bespoke journeys across Bangladesh.
🔗 [Sababa Tours](https://sababa-tours.vercel.app/)

---

## 📸 Website Screenshot

## ![Website Screenshot](./src/assets/assets_for_readme/sababa_tour.png)

## ✨ Features

- ⚡ Blazing-fast performance with **Vite + React**
- 🎨 Sophisticated luxury UI built with **Tailwind CSS**
- 🗺️ Interactive **Live Trip Planner** with dynamic pricing
- 📱 Fully responsive and mobile-first design
- 🔐 Firebase Authentication (Login & Registration)
- 📬 Functional Contact Form powered by Web3Forms
- 📖 Blog section with search and category filtering
- 🏞️ Beautiful destination showcases
- ⭐ Professional booking experience
- 🌟 Smooth animations and premium hover effects
- 📊 Animated statistics and scroll-based interactions

---

## 🛠️ Technologies Used

| Technology       | Purpose            |
| ---------------- | ------------------ |
| React.js + Vite  | Frontend Framework |
| Tailwind CSS     | Styling            |
| React Router DOM | Routing            |
| Firebase         | Authentication     |
| Web3Forms        | Contact Form       |
| React Icons      | Icons              |

---

## 📂 Project Structure

```bash
sababa-tours/
├── src/
│   ├── Components/
│   │   ├── Home/
│   │   ├── Services/
│   │   ├── About/
│   │   ├── Blogs/
│   │   ├── Contact/
│   │   ├── Navbar/
│   │   ├── Footer/
│   │   └── UI/
│   ├── Utilities/
│   ├── Firebase.init.js
│   ├── App.jsx
│   └── main.jsx
├── public/
├── package.json
├── tailwind.config.js
└── README.md
```

---

## 📍 Website Sections

- **Home** — Hero section, Trip Planner, Destinations, Stats
- **Packages** — Tour packages with search, filter, and sorting
- **Journal** — Travel blogs and FAQs
- **About** — Company story, values, and metrics
- **Contact** — Concierge form and location map
- **Authentication** — Login and Registration system

---

## ⚙️ Installation & Setup

### 1️⃣ Clone the Repository

```bash
git clone https://github.com/yourusername/sababa-tours.git
cd sababa-tours
```

### 2️⃣ Install Dependencies

```bash
npm install
```

### 3️⃣ Run Locally

```bash
npm run dev
```

Open:

```bash
http://localhost:5173
```

### 4️⃣ Build for Production

```bash
npm run build
```

---

## 🚀 Deployment

This project is deployed on **Vercel**.

---

## 🤝 Contributing

Contributions, issues, and feature requests are welcome.

### Steps to Contribute

1. Fork the repository
2. Create your feature branch

```bash
git checkout -b feature/AmazingFeature
```

3. Commit your changes

```bash
git commit -m "Add AmazingFeature"
```

4. Push to the branch

```bash
git push origin feature/AmazingFeature
```

5. Open a Pull Request

---

## 📄 License

Distributed under the **MIT License**.  
See the `LICENSE` file for more information.

---

## 👨‍💻 Author

### Mohammad Hafizur Rahman

Full-Stack Developer

📍 Chittagong, Bangladesh

🌐 Live Site:  
https://sababa-tours.vercel.app/

---

## ❤️ Acknowledgements

Made with passion for exceptional travel experiences.

    ...
    <main className="bg-[#f6f2ea] text-[#132236]">
      ...
      <section className="relative overflow-hidden bg-[#f6f2ea] py-24">
        ...
        <div className="mx-auto grid max-w-7xl gap-10 px-6 lg:grid-cols-[0.9fr_1.1fr] lg:px-10">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.22em] text-[#0f766e]">Interactive Planner</p>
            ...
            <p className="mt-5 text-sm leading-7 text-[#65758a]">
              Choose a destination, travel style, group size, and trip length. The estimate updates instantly so you can explore different possibilities.
            </p>
          </div>
          ...
          <div className="planner-shell border border-[#e7dfd0] bg-white p-4 shadow-2xl shadow-[#132236]/10 md:p-5">
            <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_320px]">
              <div>
                <div className="relative min-h-[360px] overflow-hidden bg-[#132236] md:min-h-[430px]">
                  ...
                  <div className="absolute bottom-0 max-w-xl p-6 text-white md:p-8">
                    ...
                    <h3 className="mt-2 text-4xl font-black leading-none md:text-5xl">{activeDestination.name}</h3>
                    <p className="mt-4 text-sm leading-7 text-white/76">{activeDestination.highlight}</p>
                  </div>
                </div>
                ...
                <div className="mt-4 grid gap-3 sm:grid-cols-3">
                  {destinations.map((destination, index) => (
                    <button
                      ...
                      type="button"
                    >
                      <span className="block break-words text-sm font-black leading-5">{destination.name}</span>
                      <span className={`mt-2 block break-words text-[11px] font-bold leading-4 ${selectedDestination === index ? "text-white/75" : "text-[#65758a]"}`}>
              ...
              <div className="planner-control-panel border border-[#e7dfd0] bg-[#fbf8f2] p-5">
                <p className="text-xs font-black uppercase tracking-[0.18em] text-[#0f766e]">Trip Mood</p>
                <div className="mt-4 grid gap-3">
                  {tripStyles.map((style, index) => (
                    ...
                    >
                      ...
                      <span className={`mt-1 block text-xs leading-5 ${selectedStyle === index ? "text-white/68" : "text-[#65758a]"}`}>
                        {style.pace}
                      </span>
                    </button>
                  ))}
                ...
                <div className="mt-6 grid grid-cols-2 gap-3">
                  {[
                    { label: "Travelers", value: travelers, set: setTravelers, min: 1, max: 12 },
                    { label: "Days", value: days, set: setDays, min: 2, max: 14 },
                  ].map((control) => (
                    <div key={control.label} className="border border-[#e7dfd0] bg-white p-4">
                      <p className="text-[10px] font-black uppercase tracking-[0.14em] text-[#65758a]">{control.label}</p>
                      <div className="mt-3 flex items-center justify-between">
                        <button
                          onClick={() => control.set(Math.max(control.min, control.value - 1))}
                        ...
                        <span className="text-xl font-black">{control.value}</span>
                        <button
                          onClick={() => control.set(Math.min(control.max, control.value + 1))}
                ...
                <div className="mt-6 bg-[#132236] p-5 text-white shadow-xl shadow-[#132236]/20">
                  <p className="text-xs font-bold uppercase tracking-[0.16em] text-white/55">Estimated from</p>
                  <p className="mt-1 text-4xl font-black">${estimate.toLocaleString()}</p>
                  <p className="mt-2 text-xs leading-5 text-white/58">
                    For {travelers} traveler{travelers > 1 ? "s" : ""}, {days} days, {activeStyle.name.toLowerCase()} pace.
                  </p>
                </div>
        ...
        <div className="mx-auto max-w-7xl px-6 lg:px-10">
          ...
          <div className="grid gap-6 md:grid-cols-3">
            {destinations.map((destination) => (
              <button
                key={destination.name}
                onClick={() => setSelectedDestination(destinations.findIndex((item) => item.name === destination.name))}
